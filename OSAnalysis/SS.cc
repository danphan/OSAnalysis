...
namespace Jack {
	...
	const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &jets_p4() { return object.jets_p4(); }
	const vector<float> &jets_disc() { return object.jets_disc(); }
	const int &event() { return object.event(); }
	const int &lumi() { return object.lumi(); }
	const int &run() { return object.run(); }
	const vector<int> &gen_id() { return object.gen_id(); }
	const vector<int> &gen_status() { return object.gen_status(); }
	const vector<ROOT::Math::LorentzVector<ROOT::Math::PxPyPzE4D<float> > > &gen_p4() { return object.gen_p4(); }
	const TString &filename() { return object.filename(); }
	const int &nBjets() { return object.nBjets(); }
}
